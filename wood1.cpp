...
int main()
{
    ...
    while (1) {
        ...
        if(firstStep)
        {
            direction.resize(IMAX, Pos(IMAX/2
            , JMAX/2));
        }
        ...
        for(size_t idxPac = 0; idxPac < myPacs.size(); idxPac++)
        {
            ...
            if(!firstStep && scoreMap[y][x] > 0)
            {
                ...
                cerr <<"STAY "<< pacIds[idxPac] << " " << x << " " << y <<endl;
            }
            else
            {
                const auto &pelletPos = getTargetPellet(supers, pellets, myPacs, theirPacs, idxPac, superTaken);
                direction[idxPac] = pelletPos;
                cout <<"MOVE "<< pacIds[idxPac] << " " << pelletPos.j << " " << pelletPos.i;
                cerr <<"GOTO "<< pacIds[idxPac] << " " << pelletPos.j << " " << pelletPos.i <<endl;
            }
